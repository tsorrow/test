    ...
    private void readSharedPreferences() {
        ...
        for (Map.Entry<String,Object> item_map : mapParam.entrySet()){
            ...
            if (value instanceof String){
                desc = String.format("%s\n  %s的取值为%s",desc,key,mShared.getString(key,""));
            }else if (value instanceof Integer){
                desc = String.format("%s\n  %s的取值为%s",desc,key,mShared.getInt(key,0));
            }else if (value instanceof Long){
                desc = String.format("%s\n  %s的取值为%s",desc,key,mShared.getLong(key,0l));
            }else if (value instanceof Float){
                desc = String.format("%s\n  %s的取值为%s",desc,key,mShared.getFloat(key,0.0f));
            }else if (value instanceof Boolean){
                desc = String.format("%s\n  %s的取值为%s",desc,key,mShared.getBoolean(key,false));
            }else{
